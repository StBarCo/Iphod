    ...
    case msg of
        ...
        AuthError error ->
            let
                ...
                newUser =
                    { user | error_msg = (Debug.toString error) }
        ...
        ModConfig modMsg ->
            let
                newConfig =
                    Config.update modMsg model.config
        ...
        ModLogin modMsg ->
            let
                newUser =
                    Login.update modMsg model.user
        ...
        ModFontSize modMsg ->
          let
            fsz = String.toInt model.config.fontSize |> Maybe.withDefault 12
            config = model.config
            newFsz = case (modMsg) of
              "f" -> if fsz >= 8 then fsz - 4 else 8
              "F" -> if fsz <= 25 then fsz + 4 else 24
              _   -> fsz
            newConfig = {config | fontSize = String.fromInt newFsz }
