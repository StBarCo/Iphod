...
defmodule BibleComText do
  ...
  def request(ver, vss, fnotes \\ "fnotes") do
    ...
    url =
      "https://bibles.org/v2/passages.js?q[]=#{bible_com_query(vss)}&include_marginalia=#{
        footnotes
      }&version=#{id}"

    case HTTPoison.get(url, [{"Accept", "application/jsonrequest"}],
           hackney: auth,
  ...
  def passages_to_string(passages) do
    {text, fnotes} =
      ...
      |> Enum.reduce({"", ""}, fn passage, {text, fnotes} ->
        passage_text =
          if passage["text"] |> String.length() == 0, do: "Not Available", else: passage["text"]

        text = text <> passage["display"] <> "\n" <> passage_text

        fnotes =
          if passage["footnotes"],
            do: fnotes <> footnotes_to_string(passage["footnotes"]),
            else: fnotes
