...
defmodule Iphod.DailyOptions do
  ...
  # presumes day is in Elixir.Date form
  def get_daily_options(day, dreading) do
    case resp = Repo.one(from(r in Iphod.DailyOptions, where: [date: ^day])) do
  ...
  def set_daily_options(day, dreading) do
    ...
    Repo.insert(%Iphod.DailyOptions{date: day, collect: collect, invitatory: invitatory})
    {invitatory, collect}
  end
  ...
  def collect_of_week(day, dreading) do
    c =
      cond do
        dreading.title == "Monday of Easter Week" ->
          Collects.get("easterWeek", "1").collects

        dreading.title == "Tuesday of Easter Week" ->
          Collects.get("easterWeek", "2").collects

        dreading.title == "Wednesday of Easter Week" ->
          Collects.get("easterWeek", "3").collects

        dreading.title == "Thursday of Easter Week" ->
          Collects.get("easterWeek", "4").collects

        dreading.title == "Friday of Easter Week" ->
          Collects.get("easterWeek", "5").collects

        dreading.title == "Saturday of Easter Week" ->
          Collects.get("easterWeek", "6").collects
